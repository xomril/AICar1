...
function moveWall(){
  ...
  document.getElementById("topDebug").innerHTML = "&nbsp;["+ c + "]&nbsp;Wall (" + getWallx + "," + getWally + ")<br/>&nbsp;AI(" + getAIY + "," + getAIX + ")<br/>&nbsp;Walls: " + walls + " Avoided: " + avoided + " Crash: " + crash + " Success rate: " + successRate +"%";
...
function checkCollision(){
  ...
  if(getWallX < getAIX && getAIY >= getWallY-100 && getAIY < getWallY  || getWallX < getAIX && getWallY-100 > getAIY && getWallY-100 < getAIY+50){
    ...
    document.getElementById("sensor_2").style.backgroundColor = "#ffffff";
...
function experience(){
   ...
   var getWallCenter = (getWally-100)+50;
     ...
     if(tryzone == 0 ) {
      ...
      var experienceDBother = document.getElementById("succ_" + buildvarOther).innerHTML;
      if(parseInt(experienceDBother) > parseInt(experienceDB) +parseInt(100)) {
